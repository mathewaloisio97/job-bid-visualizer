 ...
 */
export function getBreadcrumbsHtml() {
 ...
 */
export function renderJobsView(project) {
  ...
  project.jobs.forEach((job) => {
     ...
     */
    const buildMetricHtml = (title, metric, formatValue) => {
 ...
 */
export function renderBidsView(job) {
  ...
  let minCostBid = filteredBids[0] || null;
  let fastestBid = filteredBids[0] || null;
  let soonestBid = filteredBids[0] || null;
   ...
   */
  const buildMetricHtml = (title, bid, formatValue) => {
    if (!bid) {
  ...
  let html = `
        <div class="flex flex-col lg:flex-row gap-6 mb-8">
            <div class="w-full lg:w-1/4 bg-white p-6 rounded-lg shadow h-fit border border-slate-200">
                  ...
                  <div class="space-y-1.5 max-h-40 overflow-y-auto">
                     ${allVendors
                       ...
                       <label class="flex items-center text-sm text-slate-600 cursor-pointer">
                         <input type="checkbox" class="filter-vendor mr-2 rounded text-indigo-600 focus:ring-indigo-500" value="${v}" ${state.filters.vendors.includes(v) ? 'checked' : ''}>
                         ${v}
                       </label>`
                       )
                       .join('')}
                  ...
                  <div class="space-y-1.5">
                     ${allStatuses
                       ...
                       <label class="flex items-center text-sm text-slate-600 cursor-pointer capitalize">
                         <input type="checkbox" class="filter-status mr-2 rounded text-indigo-600 focus:ring-indigo-500" value="${s}" ${state.filters.statuses.includes(s) ? 'checked' : ''}>
                         ${s}
                       </label>`
                       )
                       .join('')}
                ...
                <input type="number" id="filter-cost" class="w-full mb-4 px-3 py-2 border rounded shadow-sm text-sm focus:ring-indigo-500 focus:border-indigo-500" placeholder="e.g. 15000" value="${state.filters.maxCost || ''}">
            ...
            <div class="bg-white rounded-lg shadow p-5 border border-slate-200 flex flex-col">
                <div class="flex justify-between items-start mb-2">
                    <h4 class="font-bold text-slate-800 flex items-center gap-2">
                      <span class="w-3.5 h-3.5 rounded-full shadow-sm border border-black/10" style="background-color: ${getVendorColor(bid.vendorName)}"></span>
                      ${bid.vendorName}
                    </h4>
